...
- 웹 프론트엔드 최종 미션
- 코딩테스트 끝난 후 차분한 마음으로 다시 미션을 해봅니다.
- [문제풀이 블로그](https://kit-developer.tistory.com/category/%EC%9A%B0%EC%95%84%ED%95%9C%ED%85%8C%ED%81%AC%EC%BD%94%EC%8A%A4)
- [개인 노션 블로그](https://www.notion.so/7c0ffa90976441998046e492ad069123)
- [깃 주소](https://github.com/intae92/woowacourse-precourse)

## 📃 목차

- [문제 풀이 블로그](#문제-풀이-블로그)
- [구현할 기능 목록](#구현할-기능-목록)
- [코딩테스트 문제(지하철 노선도 경로 조회 미션)](#지하철-노선도-경로-조회-미션)

---

## 🖥 문제 풀이 블로그

[블로그 바로가기](https://kit-developer.tistory.com/category/%EC%9A%B0%EC%95%84%ED%95%9C%ED%85%8C%ED%81%AC%EC%BD%94%EC%8A%A4)

---

## ✅ 구현할 기능 목록

- 제시된 역, 노선, 구간 데이터를 초기 데이터 설정 해야 한다.
- 입력
  - 입력부분을 화면에 렌더링 해야 한다
  - 최단거리, 최소시간을 선택 할 수 있어야 한다
  - 예외처리
    - 출발역과 도착역은 2글자가 이상이어야 한다
    - 존재하지 않는 역은 입력할 수 없다
    - 경로 조회 시 출발역과 도착역은 같을 수 없다
    - 경로 조회 시 출발역과 도착역은 연결되어 있지 않으면 경로를 조회할 수 없다.
- 결과
  - 결과값을 화면에 렌더링 해야 한다
  - 제시된 Dijkstra() 메서드로 결과값을 구해야 한다.
- 정상적으로 프로그램이 수행되지 않은 경우 alert으로 에러를 출력한다.

---

**아래의 내용은 출제된 코딩테스트 미션 과제 입니다.**
